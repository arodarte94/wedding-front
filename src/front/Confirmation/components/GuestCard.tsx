    ...
    return (
      <Grid item xl={4} lg={6} xs={12} padding={1} marginBottom={2}>
        <Box className={confirmationStyles.guestCard}>
          <Typography className={confirmationStyles.guestName} display={'flex'} alignItems={'center'}>
            {guest.name} {guest?.type_id == 4 && <EscalatorWarningIcon sx={{marginLeft: 1}} />}
          </Typography>
          {children}
          ...
          {guest.confirmed === true && (
            ...
            {guest?.type_id == 4 ? (
                ...
                <Box className={confirmationStyles.selectionRow}>
                <Typography className={confirmationStyles.selectionTitle} sx={{fontSize: '20px !important'}} textAlign={'center'}>
